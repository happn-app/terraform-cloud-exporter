...
var (
	WorkspacesInfo = prometheus.NewDesc(
		...
		[]string{"id", "name", "organization", "terraform_version", "created_at", "environment", "current_run", "current_run_status", "current_run_created_at", "execution_mode"}, nil,
	)
)
...
func getWorkspacesListPage(ctx context.Context, page int, organization string, config *setup.Config, ch chan<- prometheus.Metric) error {
	include := []tfe.WSIncludeOpt{tfe.WSIncludeOpt("current_run")}
